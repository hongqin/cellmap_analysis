...
for (i in 1:total) {
 ...
 if( length(tmp[,1]) > 1) {#more than 1 records, take average
   ...
   tb.gin.lenient.uniqueByAverage$Genetic.interaction.score..ε. = mean(tmp$Genetic.interaction.score..ε.)
   tb.gin.lenient.uniqueByAverage$P.value = mean(tmp$P.value)
 }  
 #else do nothing
}
```
...
totalIntermediate = length(tb.gin.intermediate[,1]);
for (i in 1:totalIntermediate) {
 ...
 if( length(tmp[,1]) > 1) {#more than 1 records, take average
   ...
   tb.gin.intermediate.uniqueByAverage$Genetic.interaction.score..ε. = mean(tmp$Genetic.interaction.score..ε.)
   tb.gin.intermediate.uniqueByAverage$P.value = mean(tmp$P.value)
 }  
 #else do nothing
}
```
...
totalstringent = length(tb.gin.stringent[,1]);
for (i in 1:totalstringent) {
 ...
 if( length(tmp[,1]) > 1) {#more than 1 records, take average
   ...
   tb.gin.stringent.uniqueByAverage$Genetic.interaction.score..ε. = mean(tmp$Genetic.interaction.score..ε.)
   tb.gin.stringent.uniqueByAverage$P.value = mean(tmp$P.value)
 }  
 #else do nothing
}
```
...
output
```{r}
write.csv(tb.gin.lenient.uniqueByAverage, "../cellmap_data/gin_lenient_uniquePairs20171109.csv", row.names = F);
write.csv(tb.gin.intermediate.uniqueByAverage, "../cellmap_data/gin_intermediate_UniquePairs20171109.csv", row.names = F);
write.csv(tb.gin.stringent.uniqueByAverage, "../cellmap_data/gin_stringen_UniquePairs20171109.csv", row.names = F);
```
